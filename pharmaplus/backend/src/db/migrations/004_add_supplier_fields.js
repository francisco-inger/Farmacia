...
function runAddSupplierFields(db) {
  const columns = db.prepare(`PRAGMA table_info(suppliers)`).all().map(c => c.name);
  
  if (!columns.includes('city')) {
    db.exec(`ALTER TABLE suppliers ADD COLUMN city TEXT DEFAULT 'Santo Domingo';`);
    console.log('  ✅ Added city column to suppliers');
  }
  if (!columns.includes('country')) {
    db.exec(`ALTER TABLE suppliers ADD COLUMN country TEXT DEFAULT 'República Dominicana';`);
    console.log('  ✅ Added country column to suppliers');
  }
  if (!columns.includes('type')) {
    db.exec(`ALTER TABLE suppliers ADD COLUMN type TEXT DEFAULT 'Nacional';`);
