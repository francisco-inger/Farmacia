...
function create(req, res) {
  ...
  const transaction = db.transaction(() => {
    ...
    let nextNum = 1;
    if (lastPurchase && lastPurchase.purchase_number) { const parts = lastPurchase.purchase_number.split('-'); nextNum = parseInt(parts[parts.length - 1]) + 1; }
    const purchaseNumber = `COM-${new Date().getFullYear()}-${String(nextNum).padStart(4, '0')}`;
    let subtotal = 0;
    for (const item of items) { subtotal += item.unit_cost * item.quantity; }
    const result = db.prepare(`INSERT INTO purchases (purchase_number, supplier_id, user_id, expected_date, subtotal, total, notes, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pendiente')`).run(purchaseNumber, supplier_id, req.user.id, expected_date||null, subtotal, subtotal, notes||null);
    const purchaseId = result.lastInsertRowid;
    for (const item of items) {
      db.prepare(`INSERT INTO purchase_items (purchase_id, product_id, quantity, unit_cost, subtotal, batch_number, expiry_date) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(purchaseId, item.product_id, item.quantity, item.unit_cost, item.unit_cost * item.quantity, item.batch_number||null, item.expiry_date||null);
    }
...
function receivePurchase(req, res) {
  ...
  if (!purchase) return res.status(404).json({ success: false, message: 'Compra no encontrada' });
  if (purchase.status === 'recibida' || purchase.status === 'cancelada') return res.status(400).json({ success: false, message: 'Compra no se puede recibir' });
  const transaction = db.transaction(() => {
    ...
    for (const item of items) {
      ...
      db.prepare(`INSERT INTO inventory_movements (product_id, movement_type, quantity, previous_stock, new_stock, reference_type, reference_id, user_id) VALUES (?, 'entrada', ?, ?, ?, 'compra', ?, ?)`).run(item.product_id, item.quantity, product.stock, newStock, req.params.id, req.user.id);
    }
    ...
    db.prepare(`INSERT INTO audit_log (user_id, user_name, action, module, description, reference_id) VALUES (?, ?, 'COMPRA_RECIBIDA', 'compras', ?, ?)`).run(req.user.id, req.user.name, `Compra ${purchase.purchase_number} recibida`, req.params.id);
  });
  try {
    transaction();
    return res.json({ success: true, message: 'Compra recibida exitosamente' });
  } catch (err) { return res.status(400).json({ success: false, message: err.message }); }
}

module.exports = { getAll, getById, create, receivePurchase };
